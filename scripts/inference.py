...
def get_embed(file):
    return np.asarray(VoiceEncoder().embed_utterance(preprocess_wav(file))).reshape(1, -1)
...
def main(args):
    ...
    if args.file_mode:
        print(f"Predicted gender : {get_prediction(model, args.file_path)}")
    else:
        df = pd.read_csv(args.csv_path, header=None, names=['file_paths'])
        df['predicted_gender'] = Parallel(n_jobs=-1)(delayed(get_prediction)(model, i) for i in tqdm(df['file_paths'].values))
